...
class TestInMemoryCache:
    ...
    async def test_can_set_values(self):
        res = await self.cache.multi_set([('service', 'service_name'), ('name', 'developer'), ['company', 'trellis']])
        assert len(res) == 3
    ...
    async def test_can_cache_values(self):
        await self.cache.multi_set([('service', 'service_name'), ('name', 'developer'), ['company', 'trellis']])
        res = await self.cache.get('service')
        assert res == 'service_name'
